...
int test_jfet_p(int argc, char *argv[])
{
    ...
    char *dut[3];
    int i = 0;
    dut[i++] = ".include \"../../../test/spice/2N5116.txt\"";
    dut[i++] = "";
    dut[i++] = NULL;
    ...
    {
        /*                   drain gate source */
        asprintf(&dut[1], "j1 /t%u /t%u /t%u sm_pjfet", probes[i][1], probes[i][0], probes[i][2]);
        ...
        assert(fabsf(result.jfet_ug - 2.85f) < 0.01f);
        assert(fabsf(result.ic_mA - 23.4f) < 0.1f);
